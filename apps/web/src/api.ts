...
export async function amendInvoice(
  ...
  const payload = await request<{ item: ApprovalAwareMutationResult<InvoiceRecord> }>("/api/invoices/amend", {
    method: "POST",
    body: JSON.stringify(input),
  });
  return payload.item;
...
export async function createInvoiceReturnAuthorization(
...
export async function recordInvoiceReturnReceipt(
  input: RecordInvoiceReturnReceiptInput,
): Promise<InvoiceRecord> {
  const payload = await request<{ item: InvoiceRecord }>("/api/invoices/return-receipts", {
    method: "POST",
    body: JSON.stringify(input),
  });
  return payload.item;
