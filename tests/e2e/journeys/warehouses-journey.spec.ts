...
test.describe.serial('Warehouses Journey: List → Create → Stock Report', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Warehouse list page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 3: Create warehouse form loads', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/warehouses/new');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.ant-form, .ant-card').first()).toBeVisible({ timeout: 10000 });
  });

  test('Step 4: Warehouse stock report loads', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/warehouses/stock-report');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.ant-table, .ant-card').first()).toBeVisible({ timeout: 10000 });
  });
