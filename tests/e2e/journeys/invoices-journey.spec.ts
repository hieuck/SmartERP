...
/**
 * Invoices & Payments User Journey E2E Test
 */
...
test.describe.serial('Invoices Journey: List → Create → Payments', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Invoice list page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 3: Create invoice form loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 4: Payments list page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 5: Create payment form loads', async ({ page }) => {
    await auth(page);
