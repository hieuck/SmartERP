...
/**
 * E-commerce User Journey E2E Test
 * 1. Login
 * 2. Product catalog page loads
 * 3. E-commerce orders page loads
...
test.describe.serial('Ecommerce Journey: Catalog → Orders', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Product catalog page loads', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/ecommerce/products');
    await page.waitForLoadState('networkidle');

    await expect(page.locator('.ant-table, .ant-card').first()).toBeVisible({ timeout: 10000 });
  });

  test('Step 3: Ecommerce orders page loads', async ({ page }) => {
    await auth(page);
