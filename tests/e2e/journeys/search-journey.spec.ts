...
test.describe.serial('Search Journey: Global Search → Results', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Search results page loads', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/search');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.ant-card, .ant-input, .ant-empty').first()).toBeVisible({
      timeout: 10000,
    });
  });

  test('Step 3: Search with query param loads results', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/search?q=product');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.ant-card').first()).toBeVisible({ timeout: 10000 });
  });

  test('Step 4: Dashboard loads without server errors', async ({ page }) => {
