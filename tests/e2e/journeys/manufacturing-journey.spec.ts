...
/**
 * Manufacturing User Journey E2E Test
 * 1. Login
 * 2. Work Orders page loads
 * 3. BOM (Bill of Materials) page loads
 * 4. Work Centers page loads
...
test.describe.serial('Manufacturing Journey: Work Orders → BOM → Work Centers', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Work Orders page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 3: BOM list page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 4: Work Centers page loads', async ({ page }) => {
    await auth(page);
