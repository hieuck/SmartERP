...
test.describe.serial('Suppliers Journey: List → Create → Purchase Orders', () => {
  test('Step 1: Login successfully', async ({ page }) => {
    await auth(page);
    expect(page.url()).toContain('/dashboard');
  });

  test('Step 2: Suppliers list page loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 3: Create supplier form loads', async ({ page }) => {
    await auth(page);
    await page.goto('/dashboard/suppliers/new');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('.ant-form, .ant-card').first()).toBeVisible({ timeout: 10000 });
  });

  test('Step 4: Purchase orders list loads', async ({ page }) => {
    await auth(page);
  ...
  test('Step 5: Create purchase order form loads', async ({ page }) => {
    await auth(page);
