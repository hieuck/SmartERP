...
describe('DashboardController', () => {
  ...
  describe('getOverview', () => {
    it('should return dashboard overview', async () => {
      ...
      expect(service.getOverview).toHaveBeenCalledWith(tenantId);
    });
  });
  ...
  describe('getSalesChart', () => {
    it('should return sales chart with default 30 days', async () => {
      ...
      const result = await controller.getSalesChart(tenantId, {});

      expect(result).toEqual(mockData);
      expect(service.getSalesChart).toHaveBeenCalledWith(tenantId, 30);
    });
  });
  ...
  describe('getTopProducts', () => {
    it('should return top 10 products', async () => {
      ...
      expect(service.getTopProducts).toHaveBeenCalledWith(tenantId, 10);
    });
  });
  ...
  describe('getTopCustomers', () => {
    it('should return top 10 customers', async () => {
      ...
      const result = await controller.getTopCustomers(tenantId, {});

      expect(result).toEqual(mockCustomers);
      expect(service.getTopCustomers).toHaveBeenCalledWith(tenantId, 10);
    });
  });
  ...
  describe('getRevenueByCategory', () => {
    it('should return revenue by category', async () => {
      ...
      const result = await controller.getRevenueByCategory(tenantId);

      expect(result).toEqual(mockRevenue);
      expect(service.getRevenueByCategory).toHaveBeenCalledWith(tenantId);
    });
  });
});
