...
describe('ValuationController (Integration)', () => {
  ...
  describe('POST /inventory/valuation/calculate-fifo', () => {
    it('should calculate FIFO cost', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/inventory/valuation/calculate-fifo')
        .send({ productId: 'prod-123', warehouseId: 'wh-123', quantity: 10 }).expect(201);
  ...
  describe('POST /inventory/valuation/add', () => {
    it('should add stock valuation', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/inventory/valuation/add')
        .send({ productId: 'prod-123', warehouseId: 'wh-123', quantity: 10, unitCost: 500000, referenceType: 'PURCHASE', referenceId: 'po-123' }).expect(201);
  ...
  describe('GET /inventory/valuation/average-cost/:productId/:warehouseId', () => {
    it('should get average cost', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/inventory/valuation/average-cost/prod-123/wh-123').expect(200);
  ...
  describe('GET /inventory/valuation/report/:productId/:warehouseId', () => {
    it('should get valuation report', async () => {
      const report = { productId: 'prod-123', totalQuantity: 100, totalValue: 50000000, averageCost: 500000 };
      valuationService.getValuationReport.mockResolvedValue(report as any);
      const response = await request(app.getHttpServer()).get('/inventory/valuation/report/prod-123/wh-123').expect(200);
