...
describe('ProductController (Integration)', () => {
  ...
  describe('POST /products', () => {
    it('should create product successfully', async () => {
      ...
      const response = await request(app.getHttpServer())
        .post('/products').set('Authorization', 'Bearer valid-token')
        .send({ sku: 'PROD-002', name: 'New Product', price: 1000000 }).expect(201);
      expect(response.body).toEqual(mockProduct);
    });

    it('should return 409 when SKU exists', async () => {
      productService.create.mockRejectedValue(new HttpException('Product with SKU \'PROD-001\' already exists', HttpStatus.CONFLICT));
      await request(app.getHttpServer()).post('/products').set('Authorization', 'Bearer valid-token')
        .send({ sku: 'PROD-001', name: 'Product' }).expect(409);
    });
  ...
  describe('GET /products', () => {
    it('should get all products', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/products').set('Authorization', 'Bearer valid-token').expect(200);
      expect(response.body).toEqual([mockProduct]);
    });

    it('should filter by status', async () => {
      productService.findByStatus.mockResolvedValue([mockProduct] as any);
      await request(app.getHttpServer()).get('/products?status=ACTIVE').set('Authorization', 'Bearer valid-token').expect(200);
    ...
    it('should filter by category', async () => {
      ...
      await request(app.getHttpServer()).get('/products?categoryId=cat-123').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /products/count', () => {
    it('should get product count', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/products/count').set('Authorization', 'Bearer valid-token').expect(200);
      expect(response.body).toBe(100);
    });

    it('should count by status', async () => {
      productService.countByStatus.mockResolvedValue(50);
      await request(app.getHttpServer()).get('/products/count?status=ACTIVE').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /products/search', () => {
    it('should search products', async () => {
      ...
      await request(app.getHttpServer()).get('/products/search?q=laptop').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /products/low-stock', () => {
    it('should get low stock products', async () => {
      ...
      await request(app.getHttpServer()).get('/products/low-stock').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('GET /products/featured', () => {
    it('should get featured products', async () => {
      ...
      await request(app.getHttpServer()).get('/products/featured').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('GET /products/sku/:sku', () => {
    it('should get product by SKU', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/products/sku/PROD-001').set('Authorization', 'Bearer valid-token').expect(200);
      expect(response.body).toEqual(mockProduct);
    });

    it('should return 404 when SKU not found', async () => {
      productService.findBySku.mockRejectedValue(new HttpException('Product with SKU \'PROD-999\' not found', HttpStatus.NOT_FOUND));
      await request(app.getHttpServer()).get('/products/sku/PROD-999').set('Authorization', 'Bearer valid-token').expect(404);
    });
  });
  ...
  describe('GET /products/:id', () => {
    it('should get product by ID', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/products/prod-123').set('Authorization', 'Bearer valid-token').expect(200);
      expect(response.body).toEqual(mockProduct);
    });

    it('should return 404 when ID not found', async () => {
      productService.findOne.mockRejectedValue(new HttpException('Product with ID prod-999 not found', HttpStatus.NOT_FOUND));
      await request(app.getHttpServer()).get('/products/prod-999').set('Authorization', 'Bearer valid-token').expect(404);
    });
  });
  ...
  describe('PUT /products/:id', () => {
    it('should update product', async () => {
      ...
      const response = await request(app.getHttpServer()).put('/products/prod-123').set('Authorization', 'Bearer valid-token')
        .send({ name: 'Updated Product' }).expect(200);
  ...
  describe('PATCH /products/:id', () => {
    it('should partially update product', async () => {
      ...
      productService.update.mockResolvedValue(updated as any);
      const response = await request(app.getHttpServer()).patch('/products/prod-123').set('Authorization', 'Bearer valid-token')
        .send({ price: 30000000 }).expect(200);
  ...
  describe('PATCH /products/:id/stock', () => {
    it('should update stock', async () => {
      ...
      await request(app.getHttpServer()).patch('/products/prod-123/stock').set('Authorization', 'Bearer valid-token')
        .send({ quantity: 100 }).expect(200);
  ...
  describe('PATCH /products/:id/stock/adjust', () => {
    it('should adjust stock', async () => {
      ...
      await request(app.getHttpServer()).patch('/products/prod-123/stock/adjust').set('Authorization', 'Bearer valid-token')
        .send({ adjustment: 10 }).expect(200);
    ...
    it('should return 400 when insufficient stock', async () => {
      productService.adjustStock.mockRejectedValue(new HttpException('Insufficient stock', HttpStatus.BAD_REQUEST));
      await request(app.getHttpServer()).patch('/products/prod-123/stock/adjust').set('Authorization', 'Bearer valid-token')
        .send({ adjustment: -1000 }).expect(400);
    });
  });
  ...
  describe('PATCH /products/:id/activate', () => {
    it('should activate product', async () => {
      ...
      await request(app.getHttpServer()).patch('/products/prod-123/activate').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('PATCH /products/:id/deactivate', () => {
    it('should deactivate product', async () => {
      ...
      await request(app.getHttpServer()).patch('/products/prod-123/deactivate').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('DELETE /products/:id', () => {
    it('should delete product', async () => {
      ...
      const response = await request(app.getHttpServer()).delete('/products/prod-123').set('Authorization', 'Bearer valid-token').expect(200);
      expect(response.body.message).toBe('Product deleted successfully');
    });

    it('should return 404 when not found', async () => {
      productService.remove.mockRejectedValue(new HttpException('Product with ID prod-999 not found', HttpStatus.NOT_FOUND));
      await request(app.getHttpServer()).delete('/products/prod-999').set('Authorization', 'Bearer valid-token').expect(404);
    });
  });
});
