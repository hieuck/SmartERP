...
describe('StockController (Integration)', () => {
  ...
  const mockStock = { id: 'stock-123', productId: 'prod-123', warehouseId: 'wh-123', quantity: 100, reservedQuantity: 10, availableQuantity: 90, tenantId: 'tenant-123' };

  beforeAll(async () => {
    const mockStockService = {
      create: jest.fn(), findAll: jest.fn(), findOne: jest.fn(), findByProduct: jest.fn(), findByWarehouse: jest.fn(),
      findByProductAndWarehouse: jest.fn(), update: jest.fn(), remove: jest.fn(), count: jest.fn(),
      getLowStockItems: jest.fn(), getOutOfStockItems: jest.fn(), getTotalValue: jest.fn(),
      adjustQuantity: jest.fn(), reserve: jest.fn(), release: jest.fn(), fulfillReservation: jest.fn(), updateStockCount: jest.fn(),
    };
  ...
  describe('POST /stock', () => {
    it('should create stock', async () => {
      ...
      const response = await request(app.getHttpServer()).post('/stock').set('Authorization', 'Bearer valid-token')
        .send({ productId: 'prod-123', warehouseId: 'wh-123', quantity: 100 }).expect(201);
  ...
  describe('GET /stock', () => {
    it('should get all stock', async () => {
      ...
      await request(app.getHttpServer()).get('/stock').set('Authorization', 'Bearer valid-token').expect(200);
    });

    it('should filter by productId', async () => {
      stockService.findByProduct.mockResolvedValue([mockStock] as any);
      await request(app.getHttpServer()).get('/stock?productId=prod-123').set('Authorization', 'Bearer valid-token').expect(200);
    ...
    it('should filter by warehouseId', async () => {
      ...
      await request(app.getHttpServer()).get('/stock?warehouseId=wh-123').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /stock/count', () => {
    it('should get stock count', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/stock/count').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /stock/low-stock', () => {
    it('should get low stock items', async () => {
      ...
      await request(app.getHttpServer()).get('/stock/low-stock').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('GET /stock/out-of-stock', () => {
    it('should get out of stock items', async () => {
      ...
      await request(app.getHttpServer()).get('/stock/out-of-stock').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('GET /stock/total-value', () => {
    it('should get total inventory value', async () => {
      ...
      const response = await request(app.getHttpServer()).get('/stock/total-value').set('Authorization', 'Bearer valid-token').expect(200);
  ...
  describe('GET /stock/product/:productId/warehouse/:warehouseId', () => {
    it('should get stock by product and warehouse', async () => {
      ...
      await request(app.getHttpServer()).get('/stock/product/prod-123/warehouse/wh-123').set('Authorization', 'Bearer valid-token').expect(200);
    });
  });
  ...
  describe('GET /stock/:id', () => {
    it('should get stock by ID', async () => {
      ...
      await request(app.getHttpServer()).get('/stock/stock-123').set('Authorization', 'Bearer valid-token').expect(200);
    });

    it('should return 404 when not found', async () => {
      stockService.findOne.mockRejectedValue(new HttpException('Stock not found', HttpStatus.NOT_FOUND));
      await request(app.getHttpServer()).get('/stock/stock-999').set('Authorization', 'Bearer valid-token').expect(404);
    });
  });
  ...
  describe('PUT /stock/:id', () => {
    it('should update stock', async () => {
      ...
      await request(app.getHttpServer()).put('/stock/stock-123').set('Authorization', 'Bearer valid-token')
        .send({ quantity: 150 }).expect(200);
    });
  });
  ...
  describe('PATCH /stock/:id/adjust', () => {
    it('should adjust stock quantity', async () => {
      ...
      await request(app.getHttpServer()).patch('/stock/stock-123/adjust').set('Authorization', 'Bearer valid-token')
        .send({ adjustment: 10, reason: 'Recount' }).expect(200);
    });
  });
  ...
  describe('PATCH /stock/:id/reserve', () => {
    it('should reserve stock', async () => {
      ...
      await request(app.getHttpServer()).patch('/stock/stock-123/reserve').set('Authorization', 'Bearer valid-token')
        .send({ quantity: 10 }).expect(200);
    });
  });
  ...
  describe('PATCH /stock/:id/release', () => {
    it('should release reserved stock', async () => {
      ...
      await request(app.getHttpServer()).patch('/stock/stock-123/release').set('Authorization', 'Bearer valid-token')
        .send({ quantity: 5 }).expect(200);
    });
  });
  ...
  describe('PATCH /stock/:id/fulfill', () => {
    it('should fulfill reservation', async () => {
      ...
      await request(app.getHttpServer()).patch('/stock/stock-123/fulfill').set('Authorization', 'Bearer valid-token')
        .send({ quantity: 10 }).expect(200);
    });
  });
  ...
  describe('PATCH /stock/:id/count', () => {
    it('should update stock count', async () => {
      ...
      await request(app.getHttpServer()).patch('/stock/stock-123/count').set('Authorization', 'Bearer valid-token')
        .send({ countedQuantity: 95 }).expect(200);
    });
  });
  ...
  describe('DELETE /stock/:id', () => {
    it('should delete stock', async () => {
      ...
      const response = await request(app.getHttpServer()).delete('/stock/stock-123').set('Authorization', 'Bearer valid-token').expect(200);
