...
export class CreateGDPRTables20260307250000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`CREATE TYPE "consent_type_enum" AS ENUM ('terms_of_service', 'privacy_policy', 'marketing_emails', 'data_processing', 'cookies');`);
    await queryRunner.query(`CREATE TYPE "export_status_enum" AS ENUM ('pending', 'processing', 'completed', 'failed', 'expired');`);
    await queryRunner.query(`CREATE TYPE "export_format_enum" AS ENUM ('json', 'csv', 'pdf');`);
    await queryRunner.query(`CREATE TYPE "deletion_status_enum" AS ENUM ('pending', 'approved', 'rejected', 'processing', 'completed', 'failed');`);
    ...
    await queryRunner.query(`CREATE INDEX "idx_consents_tenant_user_type" ON "consents"("tenantId", "userId", "type");`);
    ...
    await queryRunner.query(`CREATE INDEX "idx_export_tenant" ON "data_export_requests"("tenantId");`);
    await queryRunner.query(`CREATE INDEX "idx_export_user" ON "data_export_requests"("userId");`);
    await queryRunner.query(`CREATE INDEX "idx_export_status" ON "data_export_requests"("status");`);
    await queryRunner.query(`CREATE INDEX "idx_export_tenant_user" ON "data_export_requests"("tenantId", "userId");`);
    ...
    await queryRunner.query(`CREATE INDEX "idx_deletion_tenant" ON "data_deletion_requests"("tenantId");`);
    await queryRunner.query(`CREATE INDEX "idx_deletion_user" ON "data_deletion_requests"("userId");`);
    await queryRunner.query(`CREATE INDEX "idx_deletion_status" ON "data_deletion_requests"("status");`);
    await queryRunner.query(`CREATE INDEX "idx_deletion_tenant_status" ON "data_deletion_requests"("tenantId", "status");`);
  }
