...
describe('SupplierService', () => {
  ...
  describe('findByEmail', () => {
    it('should return a supplier by email', async () => {
      ...
      const result = await service.findByEmail('abc@supplier.com', 'tenant-1');
    ...
    it('should return null if supplier not found', async () => {
      ...
      const result = await service.findByEmail('notfound@supplier.com', 'tenant-1');
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if supplier not found', async () => {
      ...
      await expect(service.update('999', { name: 'Updated' }, 'tenant-1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw ConflictException if email already exists', async () => {
      ...
      await expect(
        service.update('1', { email: 'existing@supplier.com' }, 'tenant-1'),
    ...
    it('should allow updating to same email', async () => {
      ...
      const result = await service.update('1', { email: 'abc@supplier.com' }, 'tenant-1');
  ...
  describe('updatePaymentTerms', () => {
    it('should update payment terms', async () => {
      ...
      const result = await service.updatePaymentTerms('1', 60, 'tenant-1');

      expect(result.paymentTerms).toBe(60);
    });

    it('should throw BadRequestException for negative payment terms', async () => {
      await expect(service.updatePaymentTerms('1', -10, 'tenant-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('search', () => {
    it('should search suppliers by name', async () => {
      ...
      const result = await service.search('ABC', 'tenant-1');
  ...
  describe('getTopSuppliers', () => {
    it('should return top suppliers by balance', async () => {
      ...
      const result = await service.getTopSuppliers(3, 'tenant-1');
  ...
  describe('getSuppliersWithHighBalance', () => {
    it('should return suppliers with balance above threshold', async () => {
      ...
      const result = await service.getSuppliersWithHighBalance(1000, 'tenant-1');
