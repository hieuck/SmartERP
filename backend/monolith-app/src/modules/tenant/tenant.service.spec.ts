...
describe('TenantService', () => {
  ...
  describe('update', () => {
    ...
    it('should throw ConflictException when updating to existing code', async () => {
      ...
      await expect(service.update('tenant-1', { code: 'TNT-002' })).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('remove', () => {
    it('should throw BadRequestException when tenant has users', async () => {
      ...
      await expect(service.remove('tenant-1')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('updateStorage', () => {
    it('should update storage successfully', async () => {
      ...
      const result = await service.updateStorage('tenant-1', 500);
    ...
    it('should throw BadRequestException when storage exceeds limit', async () => {
      ...
      await expect(service.updateStorage('tenant-1', 2000)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getUsageReport', () => {
    it('should return usage report', async () => {
      ...
      const result = await service.getUsageReport('tenant-1');
